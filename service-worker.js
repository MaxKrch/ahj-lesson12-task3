...
const KEYS_CURRENT_CACHE = [KEY_LAST_CACHE, KEY_LAST_CACHE_DYNAMIC, KEY_LAST_CACHE_IMAGES];
...
const FILES_FOR_CACHE = ['./', './img/ui/bg.jpg', './app.js', './app.css', './index.html', './service-worker.js'];
const HOST_NAMES = {
...
const requestNews = async event => {
  const response = await fetch(event.request).catch(err => {
    postMessageToApp({
      type: 'Error',
...
const removeOldCache = async () => {
  ...
  return Promise.all(allCashes.filter(cache => !KEYS_CURRENT_CACHE.includes(cache)).map(cache => caches.delete(cache)));
};
